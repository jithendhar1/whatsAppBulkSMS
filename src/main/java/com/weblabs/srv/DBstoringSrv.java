...
@WebServlet("/DBstoringSrv")
@MultipartConfig(
    fileSizeThreshold = 1024 * 1024 * 2,  // 2 MB
    maxFileSize = 1024 * 1024 * 10,       // 10 MB
    maxRequestSize = 1024 * 1024 * 50     // 50 MB
)
public class DBstoringSrv extends HttpServlet {
        ...
        try {
            ...
            try (InputStream inputStream = filePart.getInputStream();
                ...
                try (Connection connection = DBUtil.provideConnection()) {
                    String insertQuery = "INSERT INTO users (sno, name, email, phno, updatedtime, isactive, usergroup) VALUES (?,?,?,?,CURDATE(),?,?)";
                    try (PreparedStatement preparedStatement = connection.prepareStatement(insertQuery)) {
                        while (rowIterator.hasNext()) {
                            Row row = rowIterator.next();
                            preparedStatement.setString(1, getPhoneNumberString(row.getCell(0)));
                            preparedStatement.setString(2, getStringCellValue(row.getCell(1)));
                            preparedStatement.setString(3, getStringCellValue(row.getCell(2)));
                            preparedStatement.setString(4, getPhoneNumberString(row.getCell(3)));
                            preparedStatement.setString(5, getPhoneNumberString(row.getCell(4)));
                            preparedStatement.setString(6, getStringCellValue(row.getCell(5)));
